...
export default function Renderer() {
  ...
  app.removeGroupItem = ({ parent_id, child_id }) => {
    refGraph[child_id].current.parent = refGraph[parent_id].current.parent
    refGraph[parent_id].current.parent.children.push(refGraph[child_id].current)

    const index = refGraph[parent_id].current.children.indexOf(
      refGraph[child_id].current
    )
    refGraph[parent_id].current.children.splice(index, 1)
  }
  app.deleteMesh = ({ uuid }) => {
    setGraph((current) => {
      delete current[uuid]
