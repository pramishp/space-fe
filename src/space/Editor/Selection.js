...
export const Selection = ({ setSelecting, setStart, setEnd, setSelection }) => {
    const { camera, scene, gl } = useThree();

    let selecting = useRef(false);
    let selectionBox = new SelectionBox(camera, scene);
    let selection = useRef([]);
    let getCoords = (event) => [(event.clientX / window.innerWidth) * 2 - 1, - (event.clientY / window.innerHeight) * 2 + 1];

    let setColors = (collection, color) => {
