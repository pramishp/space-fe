...
export default class Editor extends React.Component {
  ...
  insertSceneProps = ({ uuid, val }, notify = true) => {
        ...
        this.setState((prevState) => ({
          scenePropsGraph: { ...prevState.scenePropsGraph, ...localJsxs },
  ...
  deleteSceneProps = ({ uuid }, notify = true) => {
    ...
    this.notifyApp(
      { type: EDITOR_OPS.DELETE_SCENE_PROPS, data: { uuid } },
      notify
    )
  }
  ...
  // while selecting from the sidebar id is used and a default value is displayed
  // but while passing from the scenepropseditor an entire object is passed.
  onAddScenePropsSelected = (id) => {
    if (typeof id === 'object' && !Array.isArray(id) && id !== null) {
      console.log(id)
      this.insertSceneProps({
        uuid: id.uuid,
        val: id.val
    })
        ...
        <div style={{ height: window.innerHeight, width: '80%' }}>
          <div>
            ...
            <PropsEditor
              ...
              onAddScenePropsSelected={this.onAddScenePropsSelected.bind(this)}
